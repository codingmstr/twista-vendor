...
//                return rtrim($awsUrl, '/') . '/' . ltrim($awsBucket . '/' . $path . '/' . $data, '/');
    ...
    public static function getNextOpeningTime($schedule) {
        ...
        public static function vodafoneSms($mobile,$msg){
            
              $secureSecret = '0CF5E037A4224709A035AF92E4E886BD';


$requestParams = [
    'AccountId' => 550151031,
    'Password' => 'Vodafone@2024',
     'SenderName' => 'TWISTA EG',
    'SMSText'=>$msg
];
...
$formatedData =  "AccountId={$requestParams['AccountId']}&Password={$requestParams['Password']}";

$list ="";

if(is_array($mobile)){
    foreach($mobile as $m){
        $formatedData.="&SenderName={$requestParams['SenderName']}&ReceiverMSISDN=2{$m}&SMSText={$requestParams['SMSText']}";
        
        $list.= "<SMSList>
				<SenderName>{$requestParams['SenderName']}</SenderName>
				<ReceiverMSISDN>2{$m}</ReceiverMSISDN>
				<SMSText>{$requestParams['SMSText']}</SMSText>
			</SMSList>";
    }
}
else{
    $formatedData.="&SenderName={$requestParams['SenderName']}&ReceiverMSISDN=2{$mobile}&SMSText={$requestParams['SMSText']}";
     $list.= "<SMSList>
				<SenderName>{$requestParams['SenderName']}</SenderName>
				<ReceiverMSISDN>2{$mobile}</ReceiverMSISDN>
				<SMSText>{$requestParams['SMSText']}</SMSText>
			</SMSList>";
}

$hash = hash_hmac('sha256', $formatedData, $secureSecret);
...
$url = 'https://e3len.vodafone.com.eg/web2sms/sms/submit/';
		...
		<SubmitSMSRequest xmlns:='http://www.edafa.com/web2sms/sms/model/' xmlns:xsi='http://www.w3.org/2001/XMLSchema - instance' xsi:schemaLocation='http://www.edafa.com/web2sms/sms/model/ SMSAPI.xsd ' xsi:type='SubmitSMSRequest'>
			<AccountId>{$requestParams['AccountId']}</AccountId>
			<Password>{$requestParams['Password']}</Password>
			<SecureHash>{$hash}</SecureHash>
			{$list}
		</SubmitSMSRequest>");
		$result = curl_exec($ch);
		info($mobile.'-'.$msg.'-'.$result);
		return $result;
		curl_close($ch);
        ...
        public static function sendWhatsapp($mobile,$otp){
            
            preg_match('/\b\d{6}\b/', $otp, $matches);
    ...
    'Authorization' => 'Bearer EAAM8RMM6t1gBOxhuO4BNaeuKEuvkW6J9Ml8iyRrWzwShB5AR39lKZAZBvPL3jGt9kpemSkc86cUQAZAVFmXbzLes5gZAOdtSLXSOHj4ILKsy7cGocqJZCstV2VXaPoXXxxiSroImlEjnFZBXz4ZCd6RU0oZAOnVklvRGZAaG5DPovXmRewUZBSZAmZCdd4IzV0dslVqFoAZDZD',
    'Content-Type' => 'application/json',
])->post('https://graph.facebook.com/v20.0/477219805465693/messages', [
